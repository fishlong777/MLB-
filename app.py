...
def plot_spray_chart(df):
    ...
    for i, row in df.iterrows():
        ...
        if tx is not None:
            fig.add_trace(go.Scatter(x=[tx], y=[ty], mode='markers', marker=dict(size=18, color=row['color'], line=dict(width=1.5, color='black')), hovertemplate=f"打席: {i + 1}<br>結果: {row['events']}<br>距離: {row['hit_distance_sc']} ft", showlegend=False))
